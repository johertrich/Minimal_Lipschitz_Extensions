...
class FLOATMAP { public:
    int w, h;
    float *data;
    FLOATMAP(int w_, int h_) :w(w_), h(h_) { data = new float[w*h*3]; }
    FLOATMAP(int w_, int h_, float *data_) :w(w_), h(h_), data(data_) {}
    ~FLOATMAP() {  }
    float *operator[](int y) { return &data[y*w]; }
};

class MASKMAP { public:
    int w, h;
    float *data;
    MASKMAP(int w_, int h_) :w(w_), h(h_) { data = new float[w*h]; }
    MASKMAP(int w_, int h_, float *data_) :w(w_), h(h_), data(data_) {}
    ~MASKMAP() {  }
    float *operator[](int y) { return &data[y*w]; }
};
...
float dist(FLOATMAP *a, int ax, int ay, int bx, int by) {
    ...
    for (int dy = -patch_radius; dy <= patch_radius; dy++) {
        ...
        for (int dx = -patch_radius; dx <= patch_radius; dx++) {
            ...
            ac = *(&(*a)[ady][adx]+a->h*a->w);
            bc = *(&(*a)[bdy][bdx]+a->h*a->w);
            
            ans += ssd_similarity(ac, bc);
            
            ac = *(&(*a)[ady][adx]+a->h*a->w*2);
            bc = *(&(*a)[bdy][bdx]+a->h*a->w*2);
...
void KNN(FLOATMAP *a, MASKMAP *mask,  float * k_best_centers) {
